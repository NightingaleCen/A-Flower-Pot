...
void setup(void)
{
  ...
  loadTheCow(u8g2);//载入安科拉长角牛并初始化引脚
  wateringTime = loopTime = millis();
}
...
void loop(void)
{
  ...
  /*显示信息*/
    ...
    loopTime = millis();
    if (humidityDetect(HUMIDITY_SENSOR_PIN) < WATERING_THRESHOLD) {
      wateringTime = millis();
      digitalWrite(WATERING_PIN, LOW);
    }
  }
  if (millis() - wateringTime > waterDuration)
